...
def load_calibration():
    ...
    save_path = filedialog.askopenfilename(
        ...
        filetypes=[("Calibration", "*.npz"), ("Tous", "*.*")]
    )
    cal = np.load(save_path, allow_pickle=True)
    X, Y, vecteurs, normalisation = cal["X"], cal["Y"], cal["vecteurs"].tolist(), cal["normalisation"]
    n = len(cal.files) - 4
    deformation = [cal[f"M_{i}"] for i in range(n)]
    return deformation, X, Y, vecteurs, normalisation
...
def check_calib():
    deformation, X, Y, vecteurs, normalisation = load_calibration()
...
def reconstruction_stack():
    ...
    for jj, input_file in enumerate(input_files):
        ...
        for ii, frame in enumerate(data_raw):
            canaux = subcanals(data_raw[ii], X, Y, vecteurs)
            canaux_corriges = [cv2.warpPerspective(canaux[i],
                                                   deformation[i],
                                                   canaux[i].shape[:2][::-1],
                                                   flags=cv2.INTER_CUBIC) for i in range(6)]
